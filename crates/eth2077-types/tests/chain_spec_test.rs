use eth2077_types::chain_spec::{
    ...
    validate_chain_spec_config, ChainSpecConfig, ConsensusType, ForkEntry, ForkPhase, NetworkKind,
    SpecStatus,
};
...
fn enum_variants_are_constructible() {
    ...
    let consensus = [
        ...
        ConsensusType::CitadelOob,
        ConsensusType::Hybrid,
        ConsensusType::ProofOfAuthority,
        ConsensusType::Delegated,
        ConsensusType::Experimental,
    ];
    let status = [
        SpecStatus::Draft,
        SpecStatus::Frozen,
...
fn validation_reports_multiple_errors() {
    ...
    config.forks = vec![
        bad,
        fork("dup", ForkPhase::Activated, 10, &["EIP-2"], false),
        fork("dup", ForkPhase::Pending, 9, &["EIP-3"], true),
    ];
...
fn stats_computation_uses_unique_eips_and_version() {
    ...
    config
        .metadata
        .insert("spec_version".to_string(), "2026.2-rc1".to_string());
...
fn commitment_is_stable_and_data_sensitive() {
    ...
    left.metadata.clear();
    left.metadata
        .insert("spec_version".to_string(), "v1".to_string());
