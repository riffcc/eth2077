...
| EIP-7732 | ePBS consensus/EL integration lane | partial | Phase 1+2 scaffold landed in devnetd: slot-indexed header/envelope stores, engine registration endpoints, envelope lookup, timeliness status surface (`UNKNOWN`/`HEADER_ONLY`/`PARTIAL_REVEAL`/`REVEALED`/`LATE_REVEAL`/`WITHHELD`/`PARTIAL_WITHHOLD`/`ORPHAN_ENVELOPE`), and forkchoice rejection on withheld/late slots. Full CL+P2P integration and attestation wiring remain. |
