...
| EIP-7732 | ePBS consensus/EL integration lane | partial | Phase 1 scaffold landed in devnetd: slot-indexed header/envelope stores, engine registration endpoints, envelope lookup, and timeliness status surface (`UNKNOWN`/`HEADER_ONLY`/`PARTIAL_REVEAL`/`REVEALED`/`LATE_REVEAL`/`ORPHAN_ENVELOPE`). Full CL+P2P integration and attestation wiring remain. |
